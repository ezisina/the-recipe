...
////  TheRecipe
////
////  Created by Elena Zisina on 11/25/23.
//// Fork from
...
public struct VideoFork {
    ...
    /// Sets is ready for display
    var isReadyForDisplay: Binding<Bool?>
        
    public init(url: URL, startVideoAtSeconds:Binding<Double> = .constant(0.0), playing: Binding<Bool> = .constant(true), muted: Binding<Bool> = .constant(false), isReadyForDisplay readyForDisplay: Binding<Bool?> = .constant(false))
    {
        videoURL = url
        self.startVideoAtSeconds = startVideoAtSeconds
        isPlaying = playing
        isMuted = muted
        isReadyForDisplay = readyForDisplay
...
#if os(iOS)
extension VideoFork: UIViewControllerRepresentable {
...
#elseif os(macOS)
extension VideoFork: NSViewRepresentable {
    ...
    public func makeNSView(context: Context) -> AVPlayerView {
        ...
        videoCoordinator.observations.insert(videoView.observe(\AVPlayerView.isReadyForDisplay) { view, _  in
            DispatchQueue.main.async {
                isReadyForDisplay.wrappedValue = view.isReadyForDisplay
            }
        })
        ...
        var player: AVPlayer? {
            didSet {
                ...
                NotificationCenter.default.addObserver(self,
                                                       selector:#selector(VideoFork.VideoCoordinator.playerItemDidReachEnd),
...
//                video.isReadyForDisplay.wrappedValue = videoVo
                #else
                DispatchQueue.main.async {
...
extension VideoFork {
    ...
    public func pictureInPicturePlayback(_ value:Bool) -> VideoFork {
        var new = self
        new.allowsPictureInPicturePlayback = value
        return new
    }

    public func playbackControls(_ value: Bool) ->VideoFork {
        var new = self
        new.showsPlaybackControls = value
        return new
    }

    public func isMuted(_ value: Bool) -> VideoFork {
        return isMuted(.constant(value))
    }

    public func isMuted(_ value: Binding<Bool>) -> VideoFork {
        var new = self
        new.isMuted = value
        return new
    }

    public func isPlaying(_ value: Bool) -> VideoFork {
        let new = self
        new.isPlaying.wrappedValue = value
        return new
    }

    public func isPlaying(_ value: Binding<Bool>) -> VideoFork {
        var new = self
        new.isPlaying = value
        return new
    }
    
    public func lastPlayInSeconds(_ value: Binding<Double>) -> VideoFork {
        var new = self
        new.lastPlayInSeconds = value
        return new
    }
    
    public func backInSeconds(_ value: Binding<Double>) -> VideoFork {
        var new = self
        new.backInSeconds = value
        return new
    }
    
    public func forwardInSeconds(_ value: Binding<Double>) -> VideoFork {
        var new = self
        new.forwardInSeconds = value
        return new
    }

    public func videoGravity(_ value: AVLayerVideoGravity) -> VideoFork {
        var new = self
        new.videoGravity = value
        return new
    }

    public func loop(_ value: Bool) -> VideoFork {
        self.loop.wrappedValue = value
        return self
    }

    public func loop(_ value: Binding<Bool>) -> VideoFork {
        var new = self
        new.loop = value
        return new
    }
